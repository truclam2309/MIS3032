...
export function DecisionPanel({
  ...
  /*
   * BUG-001:
   * Only Manager can perform Approval actions.
   * Other roles can view the request but cannot approve,
  ...
  const blockedByBudget =
    Boolean(snapshot?.isOver) &&
    request.budgetCheck.state !== 'cleared';

  const baseDetails: ConfirmDetail[] = [
    {
      label: 'Request',
      value: `${request.id} · ${request.title}`
    },
    {
      label: 'Value',
      value: formatVnd(request.estimatedTotal)
    }
  ];
  ...
  if (request.status === 'error') {
    return (
      <>
        <section
          className={`${panel} border-danger-200`}
        ...
        >
          ...
          <div className="px-4 py-4">
            ...
            <p className="mt-2 text-xs text-ink-subtle">
              No approver has seen this request. Retrying resubmits exactly
              what you see below — nothing was changed.
            </p>

            <div className="mt-4 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => retrySubmission(request.id)}
                className={primary}
              >
        ...
        <section className={panel} aria-label="Draft">
          <div className="px-4 py-4">
            ...
            <p className="mt-1.5 text-sm leading-relaxed text-ink-muted">
              Nobody is waiting on this. Finish the required fields, then
              submit it from the edit screen.
            </p>
      ...
      <>
        <section className={panel} aria-label="Routing">
          <div className="px-4 py-4">
            ...
            <p className="mt-1.5 text-sm leading-relaxed text-ink-muted">
              The request has been submitted and is waiting to be assigned
              to the department approver.
            </p>

            <button
              type="button"
              onClick={() => routeForApproval(request.id)}
              className={`${primary} mt-4`}
            >
  ...
  if (request.status === 'revision-required') {
    return (
      <>
        <section
          className={`${panel} border-warn-200`}
          ...
          <div className="px-4 py-4">
            ...
            <div className="mt-5 border-t border-line/70 pt-4">
              ...
              <p className="mt-1 text-xs leading-relaxed text-ink-muted">
                If the approver only needed an explanation, answer here and
  ...
  if (request.status === 'budget-warning') {
    return (
      <>
        <section
          className={`${panel} border-warn-200`}
          aria-label="Finance budget review"
        >
          <header className="border-b border-warn-200 bg-warn-50 px-4 py-3">
            <p className="text-2xs font-semibold uppercase tracking-wider text-warn-700">
              Finance
            </p>

            <h2 className="mt-0.5 text-sm font-semibold text-warn-700">
              Budget review
            </h2>
          </header>
          ...
          <div className="px-4 py-4">
            <p className="text-sm leading-relaxed text-ink-muted">
              {snapshot
                ? `The request is ${formatVnd(
                    Math.max(0, snapshot.overBy)
                  )} above the remaining ${
                    snapshot.line.category
                  } budget. Confirm whether funding can be found, or mark the line exceeded and send it back to the manager.`
                : 'No budget line is attached to this request.'}
            </p>

            <label
              htmlFor="finance-note"
                ...
                onClick={() =>
                  setPending({
                    ...
                    details: [
                      ...
                      {
                        label: 'Budget line',
                        value: `${snapshot?.line.category} · ${snapshot?.line.period}`
                      },
                      {
                        label: 'Available',
                        value: formatVnd(snapshot?.available ?? 0)
                      },
                      {
                        label: 'Your note',
                        value: note
            ...
            <p className="mt-3 text-xs text-ink-subtle">
              Finance confirms funding only. The approval decision stays
  ...
  /*
   * BUG-001:
   * pending-approval is a Manager-only workflow step.
   *
   * If a non-Manager reaches this page directly, show the request
   * as read-only instead of rendering Approval actions.
   */
  if (request.status === 'pending-approval' && !canApprove) {
    return (
      <>
        <section
          className={`${panel} border-brand-200`}
        ...
        >
          <header className="border-b border-brand-200 bg-brand-50 px-4 py-3">
            ...
            <h2 className="mt-0.5 text-sm font-semibold text-brand-700">
              Pending approval
            </h2>
          </header>
          ...
          <div className="px-4 py-4">
            <p className="text-sm leading-relaxed text-ink-muted">
              This purchase request is waiting for Manager approval.
              You have view-only access at this workflow step.
            </p>
          </div>
        </section>
          ...
          <div className="px-4 py-4">
            {blockedByBudget ? (
              <p className="mb-4 flex items-start gap-2 rounded border border-warn-200 bg-warn-50 px-3 py-2.5 text-sm leading-relaxed text-warn-700">
                ...
                Approval is blocked: this request exceeds the remaining{' '}
                {snapshot?.line.category} budget by{' '}
                {formatVnd(snapshot?.overBy ?? 0)}. Send it to Finance,
                or reject / request a revision.
              </p>
            ) : null}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {blockedByBudget ? (
                ...
                >
                  <SendIcon className="h-4 w-4" />
                  Send to Finance for budget review
                </button>
              ) : (
                <button
                  type="button"
                  ...
                  onClick={() =>
                    setPending({
                      ...
                      irreversibleNote:
                        'An approval cannot be withdrawn in this prototype.',
                      run: () => {
                        decide(
                          request.id,
                          'approved',
              ...
              <button
                ...
                onClick={() =>
                  setPending({
                    ...
                    tone: 'danger',
                    irreversibleNote:
                      'A rejection cannot be reversed in this prototype.',
                    run: () => {
                      decide(
                        request.id,
                        'rejected',
            ...
            <p className="mt-3 text-xs text-ink-subtle">
              Every outcome here is recorded against a named person. The
              assistant cannot approve or reject a request.
            </p>
          </div>
        </section>
      ...
      <>
        ...
        >
          <header className="border-b border-ok-200 bg-ok-50 px-4 py-3">
            ...
            <p className="mt-0.5 text-xs text-ok-700">
              {request.decision?.by}
            </p>
          </header>
          ...
          <div className="px-4 py-4">
            ...
            <p className="mt-3 text-sm leading-relaxed text-ink-muted">
              Next, Procurement collects quotations against this request
              by uploading each supplier's quotation file.
            </p>
  ...
  if (
    ['po-issued', 'received', 'closed'].includes(
      request.status
    )
  ) {
