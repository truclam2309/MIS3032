...
const FILTERS: Array<{key: string;label: string;match: (s: RequestStatus) => boolean;}> = [
{ key: 'all', label: 'All', match: () => true },
{ key: 'open', label: 'In flight', match: (s) => ['submitted', 'pending-approval', 'budget-warning'].includes(s) },
{ key: 'attention', label: 'Needs me', match: (s) => ['draft', 'revision-required', 'error'].includes(s) },
{
  key: 'sourcing',
  label: 'Sourcing',
  match: (s) => ['approved', 'quotation-comparison', 'ai-recommendation', 'awarded'].includes(s)
},
{
  key: 'fulfilment',
  label: 'Fulfilment',
...
{ key: 'closed', label: 'Closed', match: (s) => ['rejected', 'closed'].includes(s) }];
...
export function Requests() {
  ...
  const visible = useMemo(() => {
    ...
    filter(
      ...
      f.match(r.status) && (
      query.trim() === '' ||
      `${r.id} ${r.title} ${r.category} ${r.requester}`.toLowerCase().includes(query.toLowerCase()))
    ).
    sort((a, b) => b.id.localeCompare(a.id));
  }, [requests, filter, query]);

  const attention = requests.filter((r) => ['error', 'revision-required'].includes(r.status));

  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-2xs font-semibold uppercase tracking-wider text-ink-subtle">Employee</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">Purchase requests</h1>
        ...
        <Link
          ...
          'border-warn-200 bg-warn-50 hover:bg-warn-50/70'}`
              ...
              <div className="flex items-center gap-2">
                <AlertTriangleIcon
              className={`h-4 w-4 ${r.status === 'error' ? 'text-danger-600' : 'text-warn-600'}`} />
              ...
              <p className="mt-1 text-xs leading-relaxed text-ink-muted">
                {r.status === 'error' ? r.errorNote : r.decision?.note}
              </p>
            </Link>
        )}
        </div> :
      ...
      <div className="mt-6 flex flex-wrap items-center gap-2">
        <div role="tablist" aria-label="Filter requests" className="flex flex-wrap gap-1 rounded border border-line bg-surface p-1">
          {FILTERS.map((f) =>
          <button
            key={f.key}
            type="button"
            ...
            filter === f.key ? 'bg-brand-50 text-brand-700' : 'text-ink-muted hover:bg-canvas'}`
            }>
        ...
        <table className="w-full text-sm">
          ...
          <thead>
            <tr className="border-b border-line bg-raised text-left text-2xs uppercase tracking-wide text-ink-subtle">
              <th scope="col" className="px-4 py-2 font-semibold">Request</th>
              <th scope="col" className="hidden px-4 py-2 font-semibold md:table-cell">Category</th>
              <th scope="col" className="px-4 py-2 text-right font-semibold">Estimated</th>
              <th scope="col" className="hidden px-4 py-2 font-semibold sm:table-cell">Needed by</th>
          ...
          <tbody>
            {visible.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-b border-line/70 last:border-0 hover:bg-raised">
                  <td className="px-4 py-3">
                    <Link to={`/requests/${r.id}`} className="font-medium text-ink hover:text-brand-600">
                      {r.title}
                    </Link>
                  ...
                  <td className="hidden px-4 py-3 text-ink-muted md:table-cell">{r.category}</td>
                  <td className="tabular px-4 py-3 text-right font-medium">
                    {formatVnd(r.estimatedTotal)}
                    {over ?
                    <span className="mt-0.5 block text-2xs font-normal text-warn-700">over remaining budget</span> :
                    null}
                  </td>
                  <td className="hidden px-4 py-3 text-ink-muted sm:table-cell">{formatDate(r.neededBy)}</td>
                  <td className="px-4 py-3">
                    <StatusBadge status={r.status} size="sm" />
                  </td>
                </tr>);
